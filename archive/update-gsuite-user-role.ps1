...
# Modules
	...
	if ($allmodules.name -notcontains "Configuration") {
		install-module -name 'Configuration' -scope AllUsers -confirm:$false -force
	} else {
		import-module "Configuration"
	}
	if ($allmodules.name -notcontains "PSGSuite") {
		install-module -name 'PSGSuite' -scope AllUsers -confirm:$false -allowclobber -force
	} else {
